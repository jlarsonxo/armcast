function loadMesh(file, scene, name, onLoadCallback = undefined) {
    ...
    loader.load(file, function (geometry) {
        ...
        let avgs = [];
        for (let i = 0; i < geometry.attributes.color.array.length; i += 3) {
            let r = geometry.attributes.color.array[i];
            let g = geometry.attributes.color.array[i + 1];
            let b = geometry.attributes.color.array[i + 2];
            let avg = (r + g + b) / 3;
            avgs.push(avg);
            geometry.attributes.color.array[i] = avg;
            geometry.attributes.color.array[i + 1] = avg;
            geometry.attributes.color.array[i + 2] = avg;
        }
